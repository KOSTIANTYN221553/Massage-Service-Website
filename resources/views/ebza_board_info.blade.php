                ...
                <div class="row content">
                    ...
                    <div class="col-sm-12 col-md-12 mt-10">
                        ...
                        <div class="text-right">
                            ...
                            <a class = "color-white write-a1 btn-danger border-none"  href = "{{url('ebza_board/write/'.$info['id'])}}" style = "color:white;">{{__('lang.수정')}}</a>
                            ...
                            @foreach($reply_list as $reply)
                                <li class="media">
                                    <div class="media-body">
                                        ...
                                        <form class = "reply_form hidden" method = "method" action="{{url("ebza_board/ajaxSaveReply")}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                                    ...
                                    <div class = "reply_reply_form-rect hidden">
                                        <form class = "reply_reply_form" method = "post" action = "{{url('/ebza_board/ajaxSaveReply')}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                        ...
                        <div class = "comment-write-rect">
                            <h3>{{__('lang.댓글작성')}}</h3>
                            <form method="POST" id = "replyForm"  action="{{url("/ebza_board/ajaxSaveReply")}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                                <input type = "hidden" name ="_token" value = "{{csrf_token()}}"/>
                                <input type = "hidden" name = "board_id" value = "{{$info['id']}}"/>
                        ...
                        <div class="col-md-3 col-sm-5 profile wow fadeInLeft"  data-wow-duration="3s" data-wow-delay="0.5s">
                            <div class="thumbnail bg-white">
                                <a href = "{{url('/ebza_board/info/'.$item['id'])}}">
                                    <img onerror = "noExitImg(this)" src="{{ correctImgPath($item['img']) }}" alt="team-image" class="img-responsive same-img">
                                </a>
                                <div class="caption color-white">
                                    <b class = "color-white word-break">
                                        ...
                                        <span class = "cursor" @if(isset($user['id']) && $item['user']['id']*1 != $user['id']*1) onclick = "menuItemClick1(this)" @endif>{{utf8_strcut(strip_tags($item['user']['nickname']), 30)}}</span>
                                        <ul class = "td-click-menu hidden">
                                    ...
                                    <p class="content color-white word-break">{{utf8_strcut(strip_tags($item['title']), 30)}}</p>
                                    <div class="divide">
                                        <div class = "row mt-10">
                                            <div class = "col-md-6">
    ...
    <script>
        ...
        function deleteReply(obj){
            confirmMsg("{{__('lang.정말 삭제하겟습니까?')}}", function(){
                ...
                param._token = _token;
                param.id = id;
                var url = "{{url("/ebza_board/deleteReply")}}";
                loading_start();
                $.post(url, param, function(data){
                    if(data.status == "1"){
