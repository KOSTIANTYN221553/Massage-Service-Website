            ...
            <div class = "col-md-9 col-xs-12 mt-10">
                    ...
                    <div class="col-md-3 col-sm-5 profile">
                        <div class="thumbnail relative">
                            @if(Sentinel::check() && ($user_info['type']*1 == 99 || $user_info['id']*1 == $item['user']['id']*1) )
                                ...
                                <a class = "board-write hidden" href = "{{url('user_board/write/'.$item['id'])}}" ><i class = "fa fa-pencil"></i></a>
                            @endif
                            <a href = "{{url('/user_board/info/'.$item['id'])}}">
                            ...
                            <div class="caption color-white">
                                <b class = "color-white">
                                    <img onerror = "noExitImg(this)" style="width:20px !important;" src="{{url($item['level_icon'])}}" @if(isset($user['id']) && $item['user']['id']*1 != $user['id']*1) onclick = "menuItemClick1(this)" @else  onclick = "userOnlyWrite(this)" @endif data-user-id ="{{$item['user_id']}}">
                                    <span class = "cursor" @if(isset($user['id']) && $item['user']['id']*1 != $user['id']*1) onclick = "menuItemClick1(this)" @else  onclick = "userOnlyWrite(this)" @endif data-user-id ="{{$item['user_id']}}">{{$item['user']['nickname']}}</span>
                ...
                <div class = "row">
                    <div class = "col-md-8 text-left col-xs-12">
                        <form id = "searchForm" action = "{{url('user_board')}}" method = "post">
                    ...
                    <div class = "col-md-4 text-right mt-10 col-xs-12" style = "margin-top:17px;">
                        @if(Sentinel::check())
                            <a class = "color-white write-a btn-danger border-none"  href = "{{url("user_board/write/0")}}">{{__('lang.글쓰기')}}</a>
                        @endif
                    </div>
                </div>
