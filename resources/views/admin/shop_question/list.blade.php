            ...
            <div class="col-lg-12">
                <div class="panel panel-primary filterable">
                    ...
                    <div class="panel-body ">
                        <form id = "searchFrm" action = "{{url("admin/shop_question")}}" method = "post">
                            ...
                            <table class="table table-bordered table-striped  table-hover">
                                ...
                                @foreach($list as $key => $item)
                                    <tr>
                                        ...
                                        <td class  ="cursor" onclick = "viewDetail(this)" data-id = "{{$item['id']}}">{{utf8_strcut(strip_tags($item['content']), 30)}}</td>
                                        <td class = "cursor" onclick = "viewDetail(this)" data-id = "{{$item['id']}}">{{getQuestionStatusStr($item['status'])}}</td>
                                        <td>
                                            <a class = "btn btn-info" href = "{{url("admin/shop_question/view/".$item['id'])}}">{{__('lang.보기')}}</a>
                                            <a class = "btn btn-danger" href = "{{url("admin/shop_question/info/".$item['id'])}}">@if($is_admin==1){{__('lang.답변')}} @else {{__('lang.수정')}} @endif</a>
                                        </td>
                                    </tr>
                                @endforeach
                                @if(count($list) == 0)
                        ...
                        <div class = "table-responsive">
                            <div class = "col-md-12 text-left">
                                ...
                                @if($user['type']*1 == 70)
                                <a type = "button" class = "btn btn-primary float-right" href = "{{url('admin/shop_question/info/0')}}"><i class = "fa fa-plus"></i>{{__('lang.글쓰기')}}</a>
                                @endif
                            </div>
                        </div>
                    </div>
...
@section('footer_scripts')
    <script type="text/javascript" src="{{ asset('assets/js/pages/admin/shop_question.js') }}" ></script>
    <script>
        function viewDetail(obj){
            var id = $(obj).attr("data-id");
            var url = "{{url('admin/shop_question/view')}}/"+id;
            loading_start();
