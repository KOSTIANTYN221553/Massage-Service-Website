    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{__('lang.매너저상세')}}</h4>
            </div>
                ...
                <div class="modal-body" >
                    <div class = "row">
                        <div class = "col-md-12" id = "detail_rect">
                            ...
                            <input name = "schedule_start_at" class = "form-control inline-block w-120" placeholder="{{__('lang.시작시간')}}">
                            <input name = "schedule_end_at" class = "form-control inline-block w-120" placeholder="{{__('lang.마감시간')}}">
                            <button class = "btn btn-primary" type = "button" id = "btn_edit" onclick = "onAddScheduleDetail()">{{__('lang.추가')}}</button>
                        </div>
                    </div>
                    <table class="table table-hover table-striped table-bordered" style = "margin-top:10px;">
                        ...
                        <tr data-id = "{{$item['id']}}">
                            ...
                            <td>
                                <a class="btn btn-raised btn-success btn-large" type = "button" onclick = "editTr(this)">{{__('lang.편집')}}</a>
                                <a class="btn btn-raised btn-danger btn-large" type = "button" onclick = "deleteTr(this)">{{__('lang.삭제')}}</a>
                            </td>
                        </tr>
                        @endforeach
                        @endif
                    ...
                    <div class = "row">
                        <div class = "col-md-12">
                            <button class = "btn btn-primary" type = "button" id = "btn_edit" onclick = "onAllRemove()">{{__('lang.전체삭제')}}</button>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button"  class="btn btn-primary" onclick = "createDetail()" >{{__('lang.확인')}}</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function initDetailRect(){
        ...
        $("#detail_rect #btn_edit").text("{{__('lang.추가')}}");
    }
