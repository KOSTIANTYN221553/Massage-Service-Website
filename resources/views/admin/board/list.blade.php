            ...
            <div class="col-lg-12">
                <div class="panel panel-primary filterable">
                    ...
                    <div class="panel-body ">
                        <form id = "searchFrm" action = "{{url("admin/board")}}" method = "post">
                                    ...
                                    <div class = "row">
                                        <div class = "col-md-4">
                                            <select class="form-control" data-width="120px"  name = "board_type">
                                                ...
                                                @foreach($board_type_list as $key =>$board)
                                                    <option value = "{{$board['id']}}" @if($board['id']*1 == $board_type*1) selected @endif>{{$board['title']}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class = "col-md-8">
                            ...
                            <table class="table table-bordered table-striped  table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class = "sorting" aria-name = "id">{{__('lang.타입')}}</th>
                                ...
                                @foreach($list as $key => $item)
                                    <tr>
                                        ...
                                        <td class  ="cursor" onclick = "viewDetail(this)" data-id = "{{$item['id']}}">{{$item['boardType']}}</td>
                                        ...
                                        <td>
                                            @if($is_admin == 1 || $user['id']*1 == $item['user_id'])
                                            <a class = "btn btn-danger" href = "{{url("admin/board/info/".$item['id'])}}">{{__('lang.수정')}}</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        <div class = "table-responsive">
                            <div class = "col-md-12 text-left">
                                ...
                                <a type = "button" class = "btn btn-primary float-right" href = "{{url('admin/board/info/0')}}"><i class = "fa fa-plus"></i>{{__('lang.글쓰기')}}</a>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('footer_scripts')
    <script type="text/javascript" src="{{ asset('assets/js/pages/admin/board.js') }}" ></script>
    <script>
        function viewDetail(obj){
            var id = $(obj).attr("data-id");
            var url = "{{url('admin/board/view')}}/"+id;
