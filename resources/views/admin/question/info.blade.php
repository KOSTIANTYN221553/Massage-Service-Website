            ...
            <div class="col-md-12">
                <div class="panel panel-primary" id="hidepanel1">
                    ...
                    <div class="panel-body">
                        <form  id = "infoForm" action = "{{url("admin/question/ajaxSaveInfo")}}" method = "post">
                            ...
                            <div class="form-group">
                                <label class="control-label" for="name">{{__('lang.문의자')}}:</label>
                                <label >{{isset($info['user']['nickname']) ? $info['user']['nickname'] :""}}</label>
                            </div>
                            <div class="form-group">
                                <label class="control-label" for="description">{{__('lang.게시판 내용')}}</label>
                                <textarea class="form-control resize_vertical" readonly  id="content" name="content" placeholder="" rows="5" style = "resize:none;">{!! strip_tags($info['content']) !!}</textarea>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{__('lang.답변')}}</label>
                                <textarea class="form-control resize_vertical"   id="answer" name="answer" placeholder="" rows="5" style = "resize:none;">{!! $info['answer'] !!}</textarea>
                            </div>
                            <!-- Form actions -->
                                ...
                                @foreach($reply_list as $reply)
                                    <li class="media">
                                        ...
                                        <div class = "reply_reply_form-rect hidden">
                                            <form class = "reply_reply_form" method = "post" action = "{{url('admin/question/ajaxSaveReply')}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                        ...
                        <div class = "comment-write-rect">
                            <h3>{{__('lang.댓글쓰기')}}</h3>
                            <form method="POST" id = "replyForm" action="{{url("admin/question/ajaxSaveReply")}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                                <input type = "hidden" name = "board_id" value = "{{$info['id']}}"/>
                    ...
                    success: function (data) {
                        if (data.status == '1') {
                            successMsg("{{__('lang.수정이 성공하였습니다.')}}", function(){
                               ...
                               if(id == "0"){
                                   window.location.href = "{{url('admin/answer')}}";
                               }else{
                                   goBack();
                               }
