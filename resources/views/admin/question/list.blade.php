            ...
            <div class="col-lg-12">
                <div class="panel panel-primary filterable">
                    ...
                    <div class="panel-body ">
                        <form id = "searchFrm" action = "{{url("admin/question")}}" method = "post">
                            ...
                            <table class="table table-bordered table-striped  table-hover">
                                ...
                                @foreach($list as $key => $item)
                                    <tr>
                                        ...
                                        <td class  ="cursor" onclick = "viewDetail(this)" data-id = "{{$item['id']}}">{{$item['user']['nickname']}}</td>
                                        ...
                                        <td>
                                            <a class = "btn btn-danger" href = "{{url("admin/question/info/".$item['id'])}}">{{__('lang.수정')}}</a>
                                        </td>
                                    </tr>
                                @endforeach
                                @if(count($list) == 0)
                        ...
                        <div class = "table-responsive">
                            <div class = "col-md-12 text-left">
                                ...
                                <a type = "button" class = "btn btn-primary float-right" href = "{{url('admin/question/info/0')}}"><i class = "fa fa-plus"></i>{{__('lang.글쓰기')}}</a>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('footer_scripts')
    <script type="text/javascript" src="{{ asset('assets/js/pages/admin/question.js?123') }}" ></script>
    <script>
        function viewDetail(obj){
            var id = $(obj).attr("data-id");
            var url = "{{url('admin/question/info')}}/"+id;
            loading_start();
