...
{{-- Page title --}}
@section('title')
    @if($id*1 == 0) {{__('lang.업소종류생성')}} @else {{__('lang.업소종류수정')}} @endif
    @parent
@stop
...
@section('content')
    ...
    <section class="content-header">
        <h1>@if($id*1 == 0) {{__('lang.업소종류생성')}} @else {{__('lang.업소종류수정')}} @endif</h1>
    </section>
    <!--section ends-->
            ...
            <div class="col-md-12">
                <div class="panel panel-primary" id="hidepanel1">
                    ...
                    <div class="panel-body">
                        <form  id = "infoForm" action = "{{url("admin/shop_type/ajaxSaveInfo")}}" method = "post">
                            <input type = "hidden" name = "_token" value = "{{csrf_token()}}"/>
                            <input type = "hidden" name = "id" value = "{{$id}}"/>
                            <div class="form-group">
                                <label class="control-label" for="name">{{__('lang.이름')}}</label>
                                <input id="name" name="title" type="text" placeholder="이름을 입력해주세요." class="form-control" value = "{{$info['title']}}">
                            </div>
                            <div class="form-group">
