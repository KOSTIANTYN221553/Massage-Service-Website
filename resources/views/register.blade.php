...
<head>
    ...
    <title>{{__('lang.회원가입')}} | {{env('SITE_NAME')}}</title>
    <!--global css starts-->
...
<div class="container">
    ...
    <div class="row">
        <div class="box">
            <h3 class="text-danger">{{__('lang.회원가입')}}</h3>
            <!-- Notifications -->
            <div id="notific">
            @include('notifications')
            </div>
            <form id = "regForm" action="{{url('user_register')}}" method="POST" >
                <!-- CSRF Token -->
                <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                <div class="form-group {{ $errors->first('email', 'has-error') }}">
                    <label style = "float:left; margin-left:0px;"> {{__('lang.아이디')}}</label>
                    <input type="text" class="form-control" id="email" name="email" placeholder="{{__('lang.영문자,숫자,_만 입력가능. 최소 3자이상 입력해주세요')}}"
                           value="{!! old('email') !!}" >
                ...
                <div class="form-group {{ $errors->first('password_confirm', 'has-error') }}">
                    ...
                    {!! $errors->first('password_confirm', '<div class="help-block">:message</div>') !!}
                </div>
                <div class="clearfix"></div>
                <div class="form-group {{ $errors->first('nickname', 'has-error') }}">
                    <label style = "float:left; margin-left:0px;"> {{__('lang.별명')}}</label>
                    <input type="text" class="form-control" id="nickname" name="nickname" placeholder="{{__('lang.한글 2자 영문4자이상 입력해주세요')}}"
                           value="{!! old('nickname') !!}" >
                    {!! $errors->first('nickname', '<div class="help-block">:message</div>') !!}
                </div>
                <div class="clearfix"></div>
                <div class="form-group {{ $errors->first('gender', 'has-error') }}">
                    <label style = "float:left; margin-left:0px;">{{__('lang.성별')}}</label>
                    <label class="radio-inline" style = "float:left;">
                        <input type="radio" name="gender" id="inlineRadio1" value="1" checked>{{__('lang.남성')}}
                    </label>
                    <label class="radio-inline" style = "float:left;">
                        <input type="radio" name="gender" id="inlineRadio2" value="2"> {{__('lang.여성')}}
                    </label>
                    {!! $errors->first('gender', '<div class="help-block">:message</div>') !!}
                </div>
                <div class="clearfix"></div>
                <div class="form-group {{ $errors->first('user_email', 'has-error') }}" style = "margin-top:10px;">
                    ...
                    <input type="email" class="form-control" id="user_email" name="user_email" placeholder="{{__('lang.이메일은 아이디 비밀번호찾기에 이용하니 정확히 기재해주시기 바랍니다')}}"
                           value="{!! old('user_email') !!}" >
                    {!! $errors->first('email', '<div class="help-block">:message</div>') !!}
                </div>
                <div class="form-group " style = "margin-top:10px;">
                    <label style = "float:left; margin-left:0px;" > {{__('lang.자기소개')}}</label>
                    <textarea class = "form-control radius-0" name ="description" rows = "2" style ="resize:none;">{!! old('description') !!}</textarea>
                </div>
                <div class = "row">
                    <div class = "col-md-4 col-lg-4">
                        <img id = "code" src = "{{url('/getDigitImage')}}"/>
                    </div>
                ...
                <div class = "row" style = "margin-top:40px;">
                    <div class = "col-md-6">
                        <button type="submit" class="btn btn-responsive button-alignment btn-danger form-control radius-0">{{__('lang.회원가입')}}</button>
                    </div>
                    <div class = "col-md-6">
                        <button type="button" onclick ="goHomepage()" class="btn  btn-responsive button-alignment btn-danger form-control radius-0">{{__('lang.취소')}}</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    ...
    function refreshCode(){
        $("#code").attr("src", "{{url('/getDigitImage')}}");
    }
    ...
    $("#regForm1").validate({
        ...
        messages: {
            email: {"required":"{{__('lang.아이디를 입력해주세요')}}", minlength:"{{__('lang.최소 8자이상 입력해주십시오')}}"},
            password: {required:"{{__('lang.암호확인을 입력해주세요')}}", minlength:"{{__('lang.최소 8자이상 입력해주십시오')}}"},
            password_confirm: {required:"{{__('lang.암호확인을 입력해주세요')}}", minlength:"{{__('lang.최소 3자이상 입력해주십시오')}}", equalTo:"{{__('lang.암호와 일치하여야 합니다')}}"},
            nickname: {"required":"{{__('lang.별명을 입력해주세요')}}", minlength:"{{__('lang.최소 2자이상 입력해주십시오')}}"},
            user_email: {"required":"{{__('lang.이메일을 입력해주세요')}}", },
        },
