                ...
                <div class="row content">
                    ...
                    <div class="col-sm-12 col-md-12">
                        <div class=" thumbnail featured-post-wide img">
                            @if(isset($info['shop']['img']))
                            <img src="{{correctImgPath($info['shop']['img'])}}" class="img-responsive head-img hidden" alt="Image">
                            @endif
                            <!-- /.blog-detail-image -->
                            <div class = "row">
                            ...
                            <div class = "row detail-header">
                                <div class = "col-md-9">
                                    ...
                                    <span class = "text-white-gray ml-5 cursor font-18"  @if(isset($user['id']) && $info['user']['id']*1 != $user['id']*1) onclick = "menuItemClick1(this)" @endif>
                        ...
                        <div class="text-right">
                            @if($info['user_id']*1 == $user['id']*1)
                                <a class = "color-white write-a1 btn-danger border-none"  href = "{{url('review/write/'.$info['id'].'/'.$info['shop_type'])}}" style = "color:white;">{{__('lang.수정')}}</a>
                            ...
                            @foreach($reply_list as $reply)
                                <li class="media">
                                    <div class="media-body">
                                        ...
                                        <form class = "reply_form hidden" method = "method" action="{{url("review/ajaxSaveReply")}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                                        ...
                                        @foreach($reply['reply'] as $reply_reply)
                                            <div class = "comment-rect">
                                                <h4 class="media-heading">
                                                    <i>{{__('lang.답글')}} :
                                                        <img onerror = "noExitImg(this)" style="width:20px !important;" src="{{url($reply_reply['user']['user_level']['icon'])}}" @if(isset($user['id']) && $reply_reply['user_id']*1 != $user['id']*1) onclick = "menuItemClick1(this)" @endif >
                                                        <span class = "cursor" @if(isset($user['id']) && $reply_reply['user_id']*1 != $user['id']*1) onclick = "menuItemClick1(this)" @endif>{{$reply_reply['user']['nickname']}}</span>
                                                        <ul class = "td-click-menu hidden">
                                                            <li><a href = "javascript:void(0);" onclick = "showSendNoteDlg(this)" data-user-id = "{{$reply_reply['user_id']}}"  >{{__('lang.쪽지보내기')}}</a></li>
                                                            <li><a href = "javascript:void(0);" onclick = "showUserInfoDlg(this)" data-user-id = "{{$reply_reply['user_id']}}" >{{__('lang.회원정보')}}</a></li>
                                                        </ul>
                                                    </i> <small class = "ml-10 text-white"> {{substr($reply_reply['created_at'],0,10)}} </small>
                                    ...
                                    <div class = "reply_reply_form-rect hidden">
                                        <form class = "reply_reply_form" method = "post" action = "{{url('/review/ajaxSaveReply')}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                        ...
                        @if(Sentinel::check())
                            <div class = "comment-write-rect">
                                <h3>{{__('lang.댓글작성')}}</h3>
                                <form method="POST" id = "replyForm"  action="{{url("/review/ajaxSaveReply")}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                                    <input type = "hidden" name ="_token" value = "{{csrf_token()}}"/>
                                    <input type = "hidden" name = "board_id" value = "{{$info['id']}}"/>
                    ...
                    <div class = "col-md-12">
                        <table class = "table table-striped">
                            ...
                            <tr>
                                ...
                                <th style = "width:20%">{{__('lang.닉네임')}}</th>
                                <th style = "width:12%" class = "hidden-xs">{{__('lang.조회수')}}</th>
                                <th style = "width:18%" class = "hidden-xs">{{__('lang.날짜')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($list as $key => $item)
                                <tr class = "cursor">
                                    ...
                                    <td  data-id ="{{$item['id']}}" class = "relative">
                                        <img onerror = "noExitImg(this)" @if(isset($user['id']) && $item['user']['id']*1 != $user['id']*1) onclick = "menuItemClick1(this)" @endif  style="width:20px !important;" src="{{url($item['level_icon'])}}">
                                    ...
                                    <td  class = "hidden-xs" onclick = "detailView(this)" data-id ="{{$item['id']}}">{{substr($item['created_at'],0,10)}}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
    ...
    <script>
        ...
        function deleteReply(obj){
            confirmMsg("{{__('lang.정말 삭제하겟습니까?')}}", function(){
                ...
                param._token = _token;
                param.id = id;
                var url = "{{url("/review/deleteReply")}}";
                loading_start();
                $.post(url, param, function(data){
                    if(data.status == "1"){
                        successMsg("{{__('lang.삭제가 성공하었습니다.')}}", function(){
