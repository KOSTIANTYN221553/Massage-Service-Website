                    ...
                    <div class="col-sm-12 col-md-12">
                        <div class=" thumbnail featured-post-wide img">
                            ...
                            <div class = "row detail-header">
                                <div class = "col-md-9 relative">
                                    ...
                                    <a class = "ml-20" href = "javascript:void(0)" data-id = "{{$info['id']}}" @if(Sentinel::check()) data-user-id = "1" @else data-user-id = "0"@endif onclick = "setFavorBoard(this)">
                                        <i class = "fa @if($is_favor*1 ==0) fa-thumbs-o-up @else fa-thumbs-up @endif"></i> <span>{{$info['favor_count']}}</span>
                                    </a>
                                </div>
                        ...
                        <div class="text-right">
                            @if($info['user_id']*1 == $user['id']*1)
                                <a class = "color-white write-a1 btn-danger border-none"  href = "{{url('board/write/'.$info['id'].'/'.$info['board_type'])}}" style = "color:white;">{{__('lang.수정')}}</a>
                        ...
                        <h3 class="comments">{{$info['reply_count']}}  {{__('lang.댓글목록')}}</h3><br />
                        <ul class="media-list">
                            @foreach($reply_list as $reply)
                                <li class="media">
                                    <div class="media-body">
                                        ...
                                        <form class = "reply_form hidden" method = "method" action="{{url("board/ajaxSaveReply")}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                                    ...
                                    <div class = "reply_reply_form-rect hidden">
                                        <form class = "reply_reply_form" method = "post" action = "{{url('/board/ajaxSaveReply')}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                        ...
                        @if(Sentinel::check())
                            <div class = "comment-write-rect">
                                <h3>{{__('lang.댓글작성')}}</h3>
                                <form method="POST" id = "replyForm"  action="{{url("/board/ajaxSaveReply")}}" accept-charset="UTF-8" class="bf" enctype="multipart/form-data">
                                    <input type = "hidden" name ="_token" value = "{{csrf_token()}}"/>
                                    <input type = "hidden" name = "board_id" value = "{{$info['id']}}"/>
                        ...
                        <div class = "table-scrollable">
                            <table class = "table  table-striped table-board">
                                ...
                                <tr>
                                    ...
                                    <th class = "text-left">{{__('lang.제목')}}</th>
                                    <th class = "text-center">{{__('lang.닉네임')}}</th>
                                    <th class = "text-center hidden-xs">{{__('lang.조회수')}}</th>
                                    <th class = "text-center hidden">{{__('lang.날짜')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($list as $key => $item)
                                    <tr class = "cursor">
                                        ...
                                        <td class = "text-left" onclick = "detailView(this)" data-id = "{{$item['id']}}">{{utf8_strcut(strip_tags($item['title']),30)}}<img onerror = "noExitImg(this)" src="{{correctImgPath($item['img'])}}" alt="team-image" style = "width:10px; height:10px;margin-bottom: 10px;margin-left: 10px;"></td>
                                        <td class = "text-center relative"  data-id = "{{$item['id']}}">
                                        ...
                                        <td class = "text-center hidden" onclick = "detailView(this)" data-id = "{{$item['id']}}">{{substr($item['created_at'], 11,5)}}</td>
                                    </tr>
                                @endforeach
                                @if(count($list) == 0)
                                    <tr>
...
@section('footer_scripts')
    <script>
        ...
        function setFavorBoard(obj){
            ...
            if(user_id == "0"){
                errorMsg("{{__('lang.먼저 로그인하십시오.')}}");
                return;
            }
            var url = "{{url('board/setFavorBoard')}}";
            var param = new Object();
            param._token = _token;
