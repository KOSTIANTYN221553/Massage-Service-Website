    ...
    public function postSignin(Request $request)
    {
        ...
        try {
            ...
            if ($user = Sentinel::authenticate($request->only(['email', 'password']), $request->get('remember-me', false))) {
                ...
                return Redirect::route("admin.dashboard")->with('success', trans('lang.login_success'));
            }

            $this->messageBag->add('email', trans('lang.account_not_found'));

        } catch (NotActivatedException $e) {
            $this->messageBag->add('email', trans('lang.account_not_found'));
        } catch (ThrottlingException $e) {
            $delay = $e->getDelay();
            $this->messageBag->add('email', trans('lang.account_not_found', compact('delay')));
        }
